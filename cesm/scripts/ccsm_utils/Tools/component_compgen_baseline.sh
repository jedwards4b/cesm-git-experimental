...
for model in ${models[*]}; do
	...
	if [ -n "$test_hist" ]; then
		...
		if [ "$compare_status" != "BFAIL_NA" ]; then
		    if [ -z "$msg" ]; then 
			print_status "$compare_status" "$compare_info" "${testcase_base}.$model.${extension}.nc : compare baseline ${model}.${extension} base with ${compare_tag}"
		    else
			print_status "$compare_status" "$compare_info" "${testcase_base}.$model.${extension}.nc : compare baseline ${model}.${extension} ($msg)"
		    fi
		...
		if [ "$generate_status" != "GFAIL_NA" ]; then
		    print_status "$generate_status" "$generate_info" "${testcase_base}.$model.${extension}.nc : generate ${model}.${extension} in baseline dir ${generate_tag}"
		fi
...
if [ -n "$compare_tag" ]; then
    if [ -z "$msg" ]; then 
	print_status "$overall_compare_status" "$compare_info" "${testcase_base} : compare baseline OVERALL"
    else
	print_status "$overall_compare_status" "$compare_info" "${testcase_base} : compare baseline OVERALL ($msg)"
    fi
fi
if [ -n "$generate_tag" ]; then
    print_status "$overall_generate_status" "$generate_info" "${testcase_base} : generate baseline OVERALL"
fi
