...
for model in ${models[*]}; do
	...
	if [[ "$testcase" =~ ^NC[KRO]_.* ]]; then
		...
		if [ "$model" != "cpl" ]; then
		    ...
		    print_status "$compare_status" "$compare_info" "${testcase_base}.${model}.${extension}.nc : compare ${model}.${extension} (.${suffix1} and .${suffix2} for _0001)"
		    ...
		    compare_status=`get_status "$compare_result"`
		    compare_info=`get_info "$compare_result"`
		    print_status "$compare_status" "$compare_info" "${testcase_base}.${model}.${extension}.nc : compare ${model}.${extension} (.${suffix1} and .${suffix2} for _0002)"
		    if [ "$compare_status" != "PASS" ]; then
			overall_status="FAIL"
		    fi
		...
		compare_status=`get_status "$compare_result"`
		compare_info=`get_info "$compare_result"`
		print_status "$compare_status" "$compare_info" "${testcase_base}.${model}.${extension}.nc : compare ${model}.${extension} (.${suffix1} for _0001 and .${suffix2} for _0001)"
		if [ "$compare_status" != "PASS" ]; then
		    overall_status="FAIL"
		fi
		...
		compare_status=`get_status "$compare_result"`
		compare_info=`get_info "$compare_result"`
		print_status "$compare_status" "$compare_info" "${testcase_base}.${model}.${extension}.nc : compare ${model}.${extension} (.${suffix1} for _0002 and .${suffix2} for _0002)"
		if [ "$compare_status" != "PASS" ]; then
		    overall_status="FAIL"
		fi
		...
		if  [[ -f ${hist1} ]] && [[ -f ${hist2} ]] ; then
		    ...
		    compare_status=`get_status "$compare_result"`
		    compare_info=`get_info "$compare_result"`
		    print_status "$compare_status" "$compare_info" "${testcase_base}.${model}.${extension}.nc : compare ${model}.${extension} (.${suffix1} and .${suffix2} files)"
		    if [ "$compare_status" != "PASS" ]; then
			overall_status="FAIL"
		    fi
		fi
		...
		if  [[ -f ${hist1} ]] && [[ -f ${hist2} ]] ; then
		    ...
		    compare_info=`get_info "$compare_result"`
		    print_status "$compare_status" "$compare_info" "${testcase_base}.${model}.${extension}.nc : compare ${model}.${extension} (.${suffix1} and .${suffix1}_${add_iop} files)"
		fi
		...
		if  [[ -f ${hist1} ]] && [[ -f ${hist2} ]] ; then
		    ...
		    compare_info=`get_info "$compare_result"`
		    print_status "$compare_status" "$compare_info" "${testcase_base}.${model}.${extension}.nc : compare ${model}.${extension} (.${suffix2} and .${suffix2}_${add_iop} files)"
		fi
...
if [ -z "$msg" ]; then 
    print_status "$overall_status" "$compare_info" "${testcase_base} : compare_hist OVERALL"
else
    print_status "$overall_status" "$compare_info" "${testcase_base} : compare_hist OVERALL ($msg)"
fi
